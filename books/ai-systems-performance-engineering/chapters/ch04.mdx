...
## FAQ
  ...
  Bucket 和层不是一一对应的。一个大层的梯度可能被拆进多个 bucket（比如一个 FFN 层的权重 200 MB，默认 25 MB/bucket
  会拆成 8 个 bucket）；反过来，多个小层的梯度也可能合并进同一个 bucket。DDP 按参数的 reverse 顺序依次填充 bucket，填满一个再开下一个，不关心层边界。
  </Accordion>
